...
class GoEmotionsProcessor(object):
    ...
    def get_examples(self, mode):
        ...
        if getattr(self.args, "task", "") in ["negative_emo", "emopillars_negative"]:
            return self._create_negative_examples(os.path.join(self.args.data_dir, file_path), mode)
        else:
            with open(os.path.join(self.args.data_dir, file_path), "r", encoding="utf-8") as f:
                lines = f.readlines()
            examples = []
...
def convert_examples_to_features(examples, label_list, max_seq_len, tokenizer):
    ...
    for example in examples:
        ...
        features.append(
            (input_ids, attention_mask, token_type_ids, torch.tensor(label_ids, dtype=torch.float))
        )
